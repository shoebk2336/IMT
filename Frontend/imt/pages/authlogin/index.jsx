...
const AuthLogin=()=>{
    ...
    const {Authenticate,AdminLogout}=useContext(Context)
...
const LoginBtn=async()=>{
       ...
       if(log.email.length==0&&log.pass.length==0){
        ...
        const Data=await fetch('http://localhost:8080/author/login/',{
            method:"POST",
        ...
        if(res.Token){
            ...
            console.log(res.Token,'authlogin')
...
const RegisterBtn=async()=>{
    ...
    if(reg.name==0&&reg.email==0&&reg.pass==0){
        ...
        const Data=await fetch('http://localhost:8080/author/singup',{
        method:"POST",
    ...
    return(
        ...
        <div>
        <h1 style={{textAlign:"center"}}>Author Login Page</h1>
